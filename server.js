...
[["sesiones_chat","ultimo_modulo","TEXT"],["sesiones_chat","pendiente","TEXT"]].forEach(([t,c,tipo]) => {
...
const twilioClient = (twilio && TWILIO_SID && TWILIO_TOKEN) ? twilio(TWILIO_SID, TWILIO_TOKEN) : null;
if (twilioClient) console.log("Twilio configurado");
...
const PAL_GAN = /(vaca|vaquillona|toro|tern|novill|rp\s|caravana|chip|pre[ñn]|tacto|destete|pesad|gdp|potrero|sanidad|vacun|parici|rodeo|semen|embri|celo|ecograf|entor|repaso|madre|padre|iatf|dosis|garrapat|aftosa|brucel|desteta|marcaci[oó]n|se[ñn]alada)/i;
...
async function responder(desde, hacia, texto) {
  if (!twilioClient) { console.log("Sin cliente Twilio, no puedo responder"); return; }
  for (const parte of partir(texto)) {
    try {
      await twilioClient.messages.create({ from: desde, to: hacia, body: parte });
    } catch (e) {
...
async function procesarMensaje({ org, usuario, texto, from, to }) {
  ...
  if (modulo === "ambiguo") {
    ...
    return responder(to, from, "¿Esto va al campo o a la plata?\n\n1️⃣ Ganadería\n2️⃣ Finanzas");
  }
  ...
  if (!base) return responder(to, from, "Ese módulo todavía no está configurado para esta empresa.");

  const url = new URL(`${base.replace(/\/$/, "")}/webhook-interno`);
  if (modulo === "gan") url.searchParams.set("campo", org.gan_campo || "");
  ...
  try {
    ...
    await responder(to, from, `${icono} ${respuesta}`);
  } catch (e) {
    console.error("webhook->modulo:", e.message);
    await responder(to, from, "No pude conectar con el sistema. Probá en un minuto.");
  }
}
...
app.post("/webhook", (req, res) => {
  ...
  // 2. Validación de firma: sin esto cualquiera puede postear mensajes falsos.
  if (twilio && TWILIO_TOKEN && process.env.VALIDAR_FIRMA !== "0") {
    const firma = req.headers["x-twilio-signature"];
    const proto = req.headers["x-forwarded-proto"] || "https";
    const urlPublica = `${proto}://${req.headers.host}${req.originalUrl}`;
    const ok = twilio.validateRequest(TWILIO_TOKEN, firma, urlPublica, req.body || {});
    if (!ok) { console.warn("Firma Twilio inválida — mensaje descartado"); return; }
  }
  ...
  procesarMensaje({ org, usuario, texto, from, to })
