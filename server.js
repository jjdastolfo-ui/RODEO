...
async function procesarMensaje({ org, usuario, texto, from, to, adjuntos = [] }) {
  ...
  const url = new URL(`${base.replace(/\/$/, "")}/webhook-interno`);
  if (modulo === "gan") {
    const lista = String(org.gan_campo || "").split(",").map(x => x.trim()).filter(Boolean);
    url.searchParams.set("campo", lista[0] || "");
    // La lista completa deja que el bot pregunte o cambie de campo.
    if (lista.length > 1) url.searchParams.set("campos_permitidos", lista.join(","));
  }
